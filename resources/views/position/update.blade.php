<x-app-layout>
    ...
    <form action='{{ route('position.update') }}' method="POST" enctype="multipart/form-data">
    @csrf
    @method("PATCH")
    <div id="login-div" class="px-[10vw] py-[10vh] grid sm:grid-cols-2 gap-5">
        <div>
            ...
            <select class="block" name="id" id="id" required>
                ...
                @foreach ($positions as $position)
                    <option {{ old("id") ? "selected" : "" }} value="{{ $position->id }}">{{ $position->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select class="block" name="position_type_id" id="position_type_id" required>
                <option value="" disabled selected class="text-gray-500">Seleccionar...</option>
                @foreach ($positionTypes as $positionType)
                    <option {{ old("position_type_id") ? "selected" : "" }} value="{{ $positionType->id }}">{{ $positionType->name }}</option>
                @endforeach
            </select>
        </div>
