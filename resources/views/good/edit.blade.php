<div class="font-sans antialiased flex flex-col justify-center items-center w-full h-full mb-0">
    ...
    <div class="px-[10vw] py-[10vh] grid sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" class="block" placeholder="Introduzca el artículo..." value={{ old('good', $good->name) }}>
                </div>
                                <div class="row-span-2 overflow-hidden flex flex-col">
                    <label for="description">Descripción</label>
                    <textarea name="description" id="description" placeholder="Escriba aquí..." maxlength="130" class="block resize-none h-full z-10">{{ old("description", $good->description) }}</textarea>
                </div>

                <div>
                    <label for="name">Cantidad</label>
                    <input type="text" id="available_amount" name="available_amount" class="block" placeholder="Introduzca la cantidad..." value={{ old('good', $good->available_amount) }}>
                </div>
                ...
                <div>
                    ...
                    <select id="inventory" name="inventory_id" class="block" placeholder="Introduzca la cantidad..." required>
                        ...
                        @foreach ($inventories as $inventory)
                            <option {{ $good->inventory_id == $inventory->id ? "selected" : "" }} value="{{ $inventory->id }}" class="inventory-option">{{ $inventory->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="flex justify-center w-full items-end p-[5vh]">
                <button type="submit" class="rounded-3xl bg-yellow-900 text-md font-bold text-white2 black_contour py-3 px-10 hover:bg-yellow-800 transition">Confirmar</button>
            </div>
        </form>
    <x-slot name="script">
