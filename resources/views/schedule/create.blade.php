...
<option value="" selected disabled>Volver</option>
    ...
    <div id="login-div" class="px-[10vw] py-[10vh] grid sm:grid-cols-2 gap-5">
        ...
        <div>
            ...
            <input type="text" required id="name" name="name" class="block" placeholder="Escriba aquí..." value="{{ old('name') }}">
        </div>
        <div>
            <label for="date">Hora de inicio</label>
            <input type="time" required id="starting_time" name="starting_time" class="block" value="{{ old('starting_time') ?? now()->format('H:i') }}">
        </div>
        <div>
            <label for="date">Hora de fin</label>
            <input type="time" required id="ending_time" name="ending_time" class="block" value="{{ old('ending_time') ?? now()->format('H:i') }}">
        </div>
    </div>
